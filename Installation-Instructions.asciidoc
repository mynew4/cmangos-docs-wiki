...
* Invoke ++cmake ../mangos++, suggested options are:
  ...
  - *IMPORTANT* ++-D INCLUDE_BINDINGS_DIR=ScriptDev2++ to include ScriptDev2
...
-----------------------
*
You may actually have to set ++CORE_PATH="../../mangos"++ (assuming default paths from this guide), if the tilde is not properly resolved into your home folder path, causing InstallFullUDB.sh to complain about not finding "~/Mangos/mangos". Tested on openSUSE 12.3.
