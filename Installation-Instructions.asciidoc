...
* Copy the pre-compiled ++C:\Mangos\mangos\contrib\extractor\ad.exe++ binary, as well.
* The GitHub repository does *not* currently contain pre-compiled binaries for vmap_assembler and vmap_extractor. You'll need to build these yourself (source code and VS solution files are in appropriately-named sub-folders of C:\Mangos\mangos\contrib), and copy the executable files created into your ++C:\World of Warcraft++ folder.
