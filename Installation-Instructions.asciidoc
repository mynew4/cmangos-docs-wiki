 ...
 * An advanced text editor: http://notepad-plus-plus.org/[Notepad++] - with syntax highlighting for most that you can wish for :)
...
We personally use some directory structure to be able to keep overview of things we have.
...
And submit this git command with return. This will take a little time, but after you will have created a subdirectory named +mangos+ into which the CMaNGOS sources are cloned.
...
On debian you can simply install the ++libboost-all-dev++ meta-package.

For more information, see http://www.boost.org/more/getting_started/index.html[here].
...
If you are using Visual Studio then http://cmangos.net/thread-7365.html[this] provides an excellent step-by-step guide on how to set up boost for Visual Studio on Windows. Adjust the info in the guide to your version of VS and Windows, of course.
...
On *nix this step is more complicated, you'll have to recompile all extrator tools.
