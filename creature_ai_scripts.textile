...
|8|EVENT_T_SPELLHIT|"spellId":spell_template#Id|"schoolMask":creature_ai_script#schoolMask|RepeatMin|RepeatMax|Expires on (Param1) "spellId":spell_template#Id or on (Param2) "schoolMask":creature_ai_script#schoolMask spellhit. Will repeat every (Param3) and (Param4)|
...
|10|EVENT_T_OOC_LOS|Hostile 0 Friendly 1|NoFriendly|RepeatMin|RepeatMax|OOC - Expires when a (Param1) Hostile 0 / Friendly 1 unit moves within (Param2) distance to creature. Will repeat every (Param3) and (Param4)|
...
|14|EVENT_T_FRIENDLY_HP|HPDeficit|Radius|RepeatMin|RepeatMax|Expires when a friendly unit has at least (param1) HP missing in (param2) radius. Will repeat every (Param3) and (Param4)|
|15|EVENT_T_FRIENDLY_IS_CC|DispelType (US)|Radius|RepeatMin|RepeatMax|Expires when a friendly unit is crowd controlled in (param2) radius. Will repeat every (Param3) and (Param4)|
|16|EVENT_T_FRIENDLY_MISSING_BUFF|"spellId":spell_template#Id|Radius|RepeatMin|RepeatMax|Expires when a friendly unit is missing aura given by (param1) "spellId":spell_template#Id in (param2) radius. Will repeat every (Param3) and (Param4)|
...
|22|EVENT_T_RECEIVE_EMOTE|TextEmotes|"ConditionType":conditions#condition_entry|NU|NU|Expires when creature receive TextEmotes [enum TextEmotes](https://github.com/cmangos/mangos-cata/blob/b76261946597de1200effc4236409a3978414056/src/game/Globals/SharedDefines.h#L1643) paired with a potential condition|
|23|EVENT_T_AURA|"spellId":spell_template#Id|AmmountInStack|RepeatMin|RepeatMax|Expires when creature have spell (Param1) aura stacks applied greater or equal to (Param2) amount. Will repeat every (Param3) and (Param4)|
...
|25|EVENT_T_SUMMONED_JUST_DIED|CreatureEntry|RepeatMin|RepeatMax|NU|Expires after creature with (Param1) "CreatureEntry":creature_template#entry died or for all spawns if (Param1 = 0). Will repeat every (Param2) and (Param3)|
|26|EVENT_T_SUMMONED_JUST_DESPAWN|CreatureEntry|RepeatMin|RepeatMax|NU|Expires before creature with (Param1) "CreatureEntry":creature_template#entry despawned or for all spawns if (Param1 = 0). Will repeat every (Param2) and (Param3)|
...
|30|EVENT_T_RECEIVE_AI_EVENT|"AIEventType":creature_ai_script#AIEventType|Sender-Entry|NU|NU|Expires when the creature receives an (Param1) "AIEventType":creature_ai_script#AIEventType, sent by (Param2 != 0) "Sender-Entry":creature_template#entry. If (Param2 = 0) then sent by any creature|
...
|3|ACTION_T_MORPH_TO_ENTRY_OR_MODEL|CreatureEntry|"modelId":creature_template#modelId1|NU|"CreatureEntry":creature_template#entry(param1) OR "modelId":creature_template#modelId1 (param2) (or 0 for both to demorph)|
...
|11|ACTION_T_CAST|"spellId":spell_template#Id|"Target":creature_ai_script#Target|"castFlags":creature_ai_script#castFlags|Creature cast spell on a target with specified "castFlags":creature_ai_script#castFlags|
|12|ACTION_T_SPAWN|CreatureEntry|"Target":creature_ai_script#Target|Duration in milliseconds|Creature spawns a creature with (Param1) CreatureEntry at Target for a given duration (infinite if zero)|
|13|ACTION_T_THREAT_SINGLE_PCT|Threat %|"Target":creature_ai_script#Target|NU|Modifies "Target":creature_ai_script#Target threat by a percent (-100 to +100)|
...
|16|ACTION_T_CAST_EVENT|CreatureEntry|"spellId":spell_template#Id|"Target":creature_ai_script#Target|Emulates spell cast on the creature for the target (must be a player) [hacky]|
|17|ACTION_T_SET_UNIT_FIELD|"EUnitField":creature_ai_script#EUnitFields|Value|"Target":creature_ai_script#Target|Sets unit field at the index to the value given for the target. More information on the field value indeces can be found at "character data":character data|
|18|ACTION_T_SET_UNIT_FLAG|"UnitFlag(s)":creature_template#unitflags|"Target":creature_ai_script#Target|NU|Sets flag(s) on the target|
|19|ACTION_T_REMOVE_UNIT_FLAG|"UnitFlag(s)":creature_template#unitflags|"Target":creature_ai_script#Target|NU|Removes flag(s) from the target|
...
|28|ACTION_T_REMOVEAURASFROMSPELL|"Target":creature_ai_script#Target|"spellId":spell_template#Id|NU|Removes all auras from a "spellId":spell_template#Id from the target|
...
|31|ACTION_T_RANDOM_PHASE_RANGE|Min Phase|Max Phase|NU|Chooses a random phase in the range specified. This number must be an integer between 0 and 31 inclusive|
|32|ACTION_T_SUMMON_ID|CreatureEntry|"Target":creature_ai_script#Target|Summon ID|Summons a creature using the data specified in the separate "creature_ai_summons":creature_ai_summons table|
|33|ACTION_T_KILLED_MONSTER|CreatureEntry|"Target":creature_ai_script#Target|NU|Simulates a kill for a "CreatureEntry":creature_template#entry given for the player from the "Target":creature_ai_script#Target|
...
|45|ACTION_T_THROW_AI_EVENT|"AIEventType":creature_ai_script#AIEventType|Radius|"Target":creature_ai_script#Target|Throws an "AIEventType":creature_ai_script#AIEventType (Param1) to friendly Npcs in range (Param2), Invoker of event is Target|
|46|ACTION_T_SET_THROW_MASK|EventTypeMask|NU|NU|Marks for which AIEvents the npc will throw AIEvents on its own.|
|47|ACTION_T_SET_STAND_STATE|"UnitStandStateType":creature_template_addon#bytes1-unit_field_bytes_10|NU|NU|Set the UnitStandStateType (Param1) of the current creature|
...
|50|ACTION_T_SET_REACT_STATE|"ReactState":creature_ai_scripts#ReactStates|NU|NU|Change ReactState of the creature|
|51|ACTION_T_PAUSE_WAYPOINTS|DoPause(0)UnPause(1)|NU|NU|Pause waypoints of creature, DoPause 0: unpause waypoint 1|
|52|ACTION_T_INTERRUPT_SPELL|"CurrentSpellType":creature_ai_script#CurrentSpellTypes|NU|NU|Interrupt spell in given slot for creature|
|53|ACTION_T_START_RELAY_SCRIPT|RelayIDorTemplate|"Target":creature_ai_script#Target|NU|Launches dbscripts_on_relay script, either static one, or when (Param1) is < 0 then random one chosen from dbscript_random_templates|
