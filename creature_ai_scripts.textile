...
All of the "events":creature_ai_script#event_type, "actions":creature_ai_script#action_type and possible "targets":creature_ai_scripts#target are summarized and described below.
...
|11|ACTION_T_CAST|"spellId":spell_template#Id|"target":creature_ai_scripts#target|"castFlags":creature_ai_script#castFlags|Creature cast spell on a target with specified "castFlags":creature_ai_script#castFlags|
|12|ACTION_T_SPAWN|CreatureEntry|"target":creature_ai_scripts#target|Duration in milliseconds|Creature spawns a creature with (Param1) CreatureEntry at Target for a given duration (infinite if zero)|
...
|16|ACTION_T_CAST_EVENT|CreatureEntry|"spellId":spell_template#Id|"target":creature_ai_scripts#target|Emulates spell cast on the creature for the target (must be a player) [hacky]|
|17|ACTION_T_SET_UNIT_FIELD|"EUnitField":creature_ai_script#EUnitFields|Value|"target":creature_ai_scripts#target|Sets unit field at the index to the value given for the target. More information on the field value indeces can be found at "character data":character data|
|18|ACTION_T_SET_UNIT_FLAG|"UnitFlag(s)":creature_template#unitflags|"target":creature_ai_scripts#target|NU|Sets flag(s) on the target|
|19|ACTION_T_REMOVE_UNIT_FLAG|"UnitFlag(s)":creature_template#unitflags|"target":creature_ai_scripts#target|NU|Removes flag(s) from the target|
...
|32|ACTION_T_SUMMON_ID|CreatureEntry|"target":creature_ai_scripts#target|Summon ID|Summons a creature using the data specified in the separate "creature_ai_summons":creature_ai_summons table|
|33|ACTION_T_KILLED_MONSTER|CreatureEntry|"target":creature_ai_scripts#target|NU|Simulates a kill for a "CreatureEntry":creature_template#entry given for the player from the "target":creature_ai_scripts#target|
...
|45|ACTION_T_THROW_AI_EVENT|"AIEventType":creature_ai_script#AIEventType|Radius|"target":creature_ai_scripts#target|Throws an "AIEventType":creature_ai_script#AIEventType (Param1) to friendly Npcs in range (Param2), Invoker of event is Target|
