...
* *16 = EVENT_T_FRIENDLY_MISSING_BUFF*: Expires when a friendly unit is missing aura's given by spell (param1) within radius (param2). Will repeat every (Param3) and (Param4) .
** Parameter 1: SpellId
** Parameter 2: Radius
** Parameter 3: RepeatMin
** Parameter 4: RepeatMax
...
|Value / Hex|_. Name|_. Description|
|1 / 0x001|CAST_INTURRUPT_PREVIOUS|Interrupts any previous spell casting.|
|2 / 0x002|CAST_TRIGGERED|Forces the cast to be instant and ignores any mana/reagents requirements.|
|4 / 0x004|CAST_FORCE_CAST|Forces spell to cast even if the target is possibly out of range or the creature is possibly out of mana|
|8 / 0x008|CAST_NO_MELEE_IF_OOM|Prevents creature from entering melee if out of mana or out of range|
|16 / 0x010|CAST_FORCE_TARGET_SELF|Forces the target to cast this spell on itself|
|32 / 0x020|CAST_AURA_NOT_PRESENT|Only casts the spell on the target if the target does not have the aura from that spell on itself already.|
|64 / 0x040|CAST_IGNORE_UNSELECTABLE_TARGET|Can target UNIT_FLAG_NOT_SELECTABLE - Needed in some scripts|
|128 / 0x080|CAST_SWITCH_CASTER_TARGET|Switches target and caster for spell cast|
|256 / 0x100|CAST_MAIN_SPELL|Marks main spell for AI Type = Range Mode (Warrior / Paladin / Mage)|
|512 / 0x200|CAST_PLAYER_ONLY|Selects only player targets - substitution for EAI not having more params|
