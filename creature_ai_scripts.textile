...
|57|NOT_IMPLEMENTED_ACTION_T_SET_RANGED_MODE|"RangeModeType":creature_ai_scripts#RangeModeType|chaseDistance|NU|Enable "RangeModeType":creature_ai_scripts#RangeModeType, distance to chase at, NU|
...
|256|NOT_IMPLEMENTED_EFLAG_RANGED_MODE_ONLY|Event only occurs in ranged mode (not yet implemented)|
|512|NOT_IMPLEMENTED_EFLAG_MELEE_MODE_ONLY|Event only occurs in melee mode (not yet implemented)|
|1024|NOT_IMPLEMENTED_EFLAG_COMBAT_ACTION|Only one per cycle (not yet implemented)|
...
|7|TARGET_T_ACTION_INVOKER_OWNER|Targets unit who is responsible for Event to occur (only works for EVENT_T_AGGRO, EVENT_T_KILL, EVENT_T_DEATH, EVENT_T_SPELLHIT, EVENT_T_OOC_LOS, EVENT_T_FRIENDLY_HP, EVENT_T_FRIENDLY_IS_CC, EVENT_T_FRIENDLY_MISSING_BUFF, EVENT_T_RECEIVE_EMOTE, EVENT_T_RECEIVE_AI_EVENT)|
...
|15|NOT_IMPLEMENTED_TARGET_T_NONE|Default spell target - sets nullptr which should be most common spell fill|
...
|256|NOT_IMPLEMENTED_CAST_MAIN_SPELL|Marks main spell for AI Type = Action 57 ACTION_T_SET_RANGED_MODE|
|512|NOT_IMPLEMENTED_CAST_PLAYER_ONLY|Selects only player targets - substitution for EAI not having more params|
|1024|NOT_IMPLEMENTED_CAST_DISTANCE_YOURSELF|If spell with this cast flag hits main aggro target, caster distances himself - EAI only|
